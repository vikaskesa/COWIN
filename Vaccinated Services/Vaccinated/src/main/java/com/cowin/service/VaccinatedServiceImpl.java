...
public class VaccinatedServiceImpl implements VaccinatedService {
	...
	@Override
	public List<Vaccinated> getAllVaccinatedDetailsByDose(int doseNumber)throws VaccinatedDetailsNotFoundByDoseNumberException {
		
		List<VaccinatedEntity> vaccinatedList = vaccinatedRepository.findByDoseNumber(doseNumber);
		
		if (vaccinatedList.isEmpty()) {
			throw new VaccinatedDetailsNotFoundByDoseNumberException(
					"Sorry! vaccinated details is not existing with dosenumber: " + doseNumber);
		}
